...
# Uses ProbSparse self-attention for reduced complexity
##############################################################################
...
echo "##############################################################################"
echo "# Training Informer Model on All Datasets"
echo "##############################################################################"

for dataset in "${DATASETS[@]}"; do
    for horizon in "${HORIZONS[@]}"; do
        echo "Training: Informer on $dataset for H=$horizon"
        ...
        python run.py \
            ...
            --model_id "Informer_${dataset}_H${horizon}" \
            --model $MODEL \
            --data custom \
            --features $FEATURES \
            --target $TARGET \
...
echo "Informer training completed for all datasets!"
